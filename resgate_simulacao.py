...
# --------------------------------------
def bfs(
    origem: Node, destino: Node, adj: Dict[Node, List[Tuple[Node, float]]]
) -> Optional[List[Node]]:
...
# --------------------------------------
def caminho_custo(
    caminho: List[Node], adj: Dict[Node, List[Tuple[Node, float]]]
) -> float:
...
def resumo(resultados: List[dict]) -> dict:
    ...
    dj_custos = [
        r["dijkstra_custo"] for r in resultados if r["dijkstra_custo"] is not None
    ]
    bfs_passos = [r["bfs_passos"] for r in resultados if r["bfs_passos"] is not None]
    dj_passos = [
        r["dijkstra_passos"] for r in resultados if r["dijkstra_passos"] is not None
    ]
